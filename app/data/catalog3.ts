...
export type CodingGroup = "Komfort" | "Licht" | "Infotainment" | "Assistenz" | "Fahrdynamik" | "Diagnose";
...
function normalizeName(name:string):string {
  const aliases:Record<string,string> = {
    "Auto-Lock / Auto-Unlock aktivieren":"Auto-Lock / Auto-Unlock aktivieren",
    "Auto-Lock aktivieren":"Auto-Lock aktivieren",
    "Auto-Unlock aktivieren":"Auto-Unlock aktivieren",
    "Coming Home / Leaving Home anpassen":"Coming Home / Leaving Home anpassen",
    "Coming-Home- und Leaving-Home-Zeit einstellen":"Coming Home / Leaving Home Zeit einstellen",
    "Beifahrerspiegelabsenkung beim Rückwärtsfahren":"Beifahrerspiegelabsenkung bei Rückwärtsfahrt",
    "Spiegelabsenkung Beifahrer bei Rückwärtsfahrt":"Beifahrerspiegelabsenkung bei Rückwärtsfahrt",
    "Spiegelabsenkung Beifahrer bei Rückwärtsfahrt aktivieren":"Beifahrerspiegelabsenkung bei Rückwärtsfahrt",
    "Rückleuchten mit Tagfahrlicht aktivieren":"Heckleuchten zusammen mit Tagfahrlicht aktivieren",
...
function priceForName(name:string):number {
  ...
  if(n.includes("effizienzassistent")) return 59;
  if(n.includes("surround-view")) return 49;
  if(n.includes("wireless carplay") || n.includes("carplay / android auto")) return 49;
  if(n.includes("hidden menu") || n.includes("developer mode") || n.includes("green menu")) return 25;
  ...
  if(n.includes("spiegelanklappen") || n.includes("elektrisch anklappbare spiegel")) return 29;
  ...
  if(n.includes("xds") || n.includes("berganfahrassistent") || n.includes("bremsscheibentrocknung") || n.includes("esc sport") || n.includes("lenkungskennlinie")) return 25;
  if(n.includes("regenschliessen") || n.includes("fussraumbeleuchtung") || n.includes("sitzheizung") || n.includes("offroad-anzeige") || n.includes("fahrschulmodus")) return 25;
  if(n.includes("zeigertest") || n.includes("staging") || n.includes("gurtwarner") || n.includes("nachtankmenge") || n.includes("auto-lock") || n.includes("auto-unlock") || n.includes("komfortblinken") || n.includes("tränenwischen") || n.includes("heckwischer") || n.includes("ganganzeige")) return 15;
  if(n.includes("öltemperatur") || n.includes("laptimer") || n.includes("rundenzähler") || n.includes("coming home") || n.includes("leaving home") || n.includes("tagfahrlicht") || n.includes("heckleuchten") || n.includes("komfortöffnung") || n.includes("komfortschliessung") || n.includes("fenster per") || n.includes("akustische quittierung") || n.includes("hornquittierung") || n.includes("startlogo") || n.includes("tacho-darstellung") || n.includes("skin der") || n.includes("verbrauchsanzeige")) return 19;
  return 25;
}
...
function groupForName(name:string):CodingGroup {
  const n=name.toLowerCase();
  if(["verkehrszeichen","lane assist","fernlichtassistent","acc ","rückfahrkamera","einparkhilfe","surround-view","effizienzassistent"].some(k=>n.includes(k))) return "Assistenz";
  if(["tagfahrlicht","heckleuchten","rückleuchten","coming home","leaving home","abbiegelicht","nebelscheinwerfer","scheinwerfer","parklicht","innenlicht","fussraumbeleuchtung","ambientebeleuchtung"].some(k=>n.includes(k))) return "Licht";
  if(["mmi","infotainment","carplay","android auto","hidden menu","green menu","developer mode","startlogo","virtual cockpit","tacho","multifunktionsanzeige","fahrschulmodus","offroad-anzeige","welcome sound"].some(k=>n.includes(k))) return "Infotainment";
  if(["xds","esc sport","berganfahrassistent","bremsscheibentrocknung","fahrprofil","drive select","lenkung","soundaktor","start-stopp"].some(k=>n.includes(k))) return "Fahrdynamik";
  return "Komfort";
}

const normalizedNames = Array.from(new Set(ascodingCodingNames.map(normalizeName)));
const sourceCodings:Coding[] = normalizedNames.map((name,index)=>({
  id:`asc-${index+1}`,
  name,
  price:priceForName(name),
  category:categoryForName(name),
  uiGroup:groupForName(name),
  interfaceInfo:"VCDS; je nach Steuergerät und Softwarestand ggf. VCP oder ODIS nach Vorprüfung",
...
export const codingCatalog:Coding[] = [
  {id:"diagnose",name:"Fehlerdiagnose Steuergeräte",price:39,category:"Standard-Codierungen",uiGroup:"Diagnose",interfaceInfo:"VCDS / VCP / OBD11 – abhängig vom Fahrzeug",requirements:"Fehlerspeicher auslesen, Steuergeräte prüfen und Fehlerbild vorbewerten."},
  ...sourceCodings,
];
...
export const vehicles:Vehicle[] = [
  ...
  {brand:"Audi",model:"e-tron GE",startYear:2018,endYear:2026,platform:"MLBevo",sourceUrl:`${A}/audi_etron/`},

  {brand:"Škoda",model:"Octavia 2 1Z",startYear:2004,endYear:2013,platform:"PQ35",sourceUrl:`${A}/skoda-octavia-2-1z/`},
  {brand:"Škoda",model:"Octavia 3 5E",startYear:2012,endYear:2017,platform:"MQB",sourceUrl:`${A}/skoda-octavia-3-5e/`},
  ...
  {brand:"Škoda",model:"Superb 3V",startYear:2015,endYear:2023,platform:"MQB",sourceUrl:`${A}/skoda-superb-3v/`},
...
export function codingsForVehicle(vehicle:Vehicle):string[] {
  ...
  const names=(ascodingModelCodings[key] || []).map(normalizeName);
  const ids=names.map(n=>idByName.get(n)).filter((id):id is string=>!!id);
  return ["diagnose",...Array.from(new Set(ids))];
}

export const brands=["Volkswagen","Audi","Škoda","SEAT / CUPRA"];
export const codingGroups:CodingGroup[]=["Komfort","Licht","Infotainment","Assistenz","Fahrdynamik","Diagnose"];
