...
function codingsFromCodierlisten(vehicle: Vehicle, year: number, isSfd1: boolean, isSfd2: boolean) {
  if (isSfd2) return [] as UnifiedCodingEntry[];

  const platformSource = codingSources.find((source) => source.platform === vehicle.platform);
  if (platformSource) {
...
export default function BookingConfigurator() {
  ...
  const available = useMemo(() => {
    if (!selectedVehicle || !year) return [] as UnifiedCodingEntry[];
    return codingsFromCodierlisten(selectedVehicle, year, isSfd1, isSfd2);
  ...
  const formattedPrepay = formatEuro(prepay);
  ...
  if (mode === "remote") {
    ...
    calParams.set("zahlung", "Bar, PayPal oder Sofortüberweisung (beim Termin)");
  }
  ...
  const calButton = (className: string, saveBooking = false) =>
    bookingDisabled ? (
      <button
        type="button"
        disabled
        className={`inline-flex cursor-not-allowed flex-wrap items-center justify-center gap-x-2 gap-y-1 rounded-xl border border-slate-300 bg-slate-200 px-5 py-3 font-semibold text-slate-500 shadow-none ${className}`}
      >
        <CalendarDays className="h-5 w-5 shrink-0" />
        <span>Termin mit Daten an Cal.com übergeben</span>
        {mode === "onsite" ? (
          <span className="whitespace-nowrap rounded-lg bg-slate-300 px-2 py-1 text-slate-700">
      ...
      <a
        href={calUrl}
        onClick={saveBooking ? savePendingBooking : undefined}
        target="_blank"
        rel="noreferrer"
        className={`btn-primary flex-wrap gap-x-2 gap-y-1 ${className}`}
      >
        <CalendarDays className="h-5 w-5 shrink-0" />
        <span>Termin mit Daten an Cal.com übergeben</span>
        {mode === "onsite" ? (
          <span className="whitespace-nowrap rounded-lg bg-white/15 px-2 py-1">
            Gesamtsumme: {formattedTotal} €
          </span>
        ) : null}
      </a>
  ...
  const mobileCalButtonContent = (
    <>
      <span className="leading-tight">Termin mit Daten an Cal.com übergeben</span>
      {mode === "onsite" ? (
        <span className="mt-1 text-xs font-black">Gesamtsumme: {formattedTotal} €</span>
      ) : null}
    </>
  );
  ...
  const mobileCalButton = bookingDisabled ? (
    <button
      type="button"
      disabled
      className="inline-flex min-h-16 w-full cursor-not-allowed flex-col items-center justify-center rounded-xl border border-slate-300 bg-slate-200 px-3 py-2 text-center text-[11px] font-semibold text-slate-500"
    >
      {mobileCalButtonContent}
    </button>
  ) : (
    <a
      href={calUrl}
      onClick={mode === "remote" ? savePendingBooking : undefined}
      target="_blank"
      rel="noreferrer"
      className="inline-flex min-h-16 w-full flex-col items-center justify-center rounded-xl bg-blue-600 px-3 py-2 text-center text-[11px] font-semibold text-white shadow-sm transition hover:bg-blue-700"
    >
      {mobileCalButtonContent}
    </a>
  );
  ...
  return (
    <div className="space-y-5 sm:space-y-8">
      <section className="card p-4 sm:p-8">
        <div className="text-xs font-bold uppercase tracking-[.16em] text-blue-600 sm:text-sm sm:tracking-[.18em]">
          1 · Terminart
        </div>
        <div className="mt-4 grid gap-3 sm:mt-5 sm:gap-4 md:grid-cols-2">
          <button
            ...
            className={`min-h-[132px] rounded-2xl border p-4 text-left transition duration-200 hover:-translate-y-0.5 hover:border-blue-400 hover:shadow-md sm:min-h-[148px] sm:p-5 ${
              mode === "onsite" ? "border-blue-600 bg-blue-50" : "border-slate-200 bg-white"
            }`}
          >
            <MapPin className="h-7 w-7 text-blue-600" />
            <b className="mt-3 block">Vor Ort in Leipzig-Süd</b>
            <span className="text-sm text-slate-600">Schenkendorfstraße 33, 04275 Leipzig</span>
          </button>
          <button
            type="button"
            onClick={() => setMode("remote")}
            className={`min-h-[132px] rounded-2xl border p-4 text-left transition duration-200 hover:-translate-y-0.5 hover:border-blue-400 hover:shadow-md sm:min-h-[148px] sm:p-5 ${
              mode === "remote" ? "border-blue-600 bg-blue-50" : "border-slate-200 bg-white"
            }`}
          >
            <Laptop className="h-7 w-7 text-blue-600" />
            <b className="mt-3 block">Remote-Codierung</b>
            <span className="mt-1 block text-sm leading-6 text-slate-600">
              Mit eigenem Diagnoseinterface, PC/Laptop, stabiler Internetverbindung und vereinbarter Remote-Software.
            </span>
          </button>
        </div>
        {mode === "remote" ? (
          <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm leading-6 text-slate-700 sm:mt-5 sm:p-5 sm:leading-7">
      ...
      <section className="card p-4 sm:p-8">
        ...
        <h3 className="mt-2 text-xl font-black leading-tight sm:text-2xl">Marke, Modell und Baujahr</h3>
        <div className="mt-4 grid gap-3 sm:mt-5 sm:gap-4 md:grid-cols-3">
          ...
          <label>
            ...
            >
              ...
              {models.map((vehicle) => (
                <option key={`${vehicle.brand}-${vehicle.model}`} value={vehicle.model}>
                  {vehicle.model}
                </option>
              ))}
            </select>
        ...
        <div className="mt-4 grid gap-3 sm:gap-4 md:grid-cols-[1fr_2fr]">
          <div>
            ...
            {vin.length > 0 && vin.length !== 17 ? (
              <p className="mt-2 text-xs font-semibold text-amber-600">
                FIN muss 17 Zeichen enthalten · {vin.length}/17
              </p>
            ) : null}
            {vin.length === 17 ? (
          ...
          {selectedVehicle ? (
            <div className="rounded-xl bg-slate-50 p-3 text-sm leading-6 text-slate-600 sm:p-4">
              Erkannt: <strong>{platformLabels[selectedVehicle.platform] ?? selectedVehicle.platform}</strong>. {" "}
              {platformSource
                ? "Die umfangreiche Codierliste der Codierübersicht wird verwendet."
                : "Die fahrzeugbezogene Bestandsliste der Codierübersicht wird verwendet."}
            </div>
          ) : (
            <div className="rounded-xl bg-slate-50 p-3 text-sm leading-6 text-slate-500 sm:p-4">
              Bitte zuerst Marke und Modell auswählen.
            </div>
          )}
        </div>

        {isSfd1 ? (
          <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm leading-6 text-blue-900 sm:mt-5 sm:p-4">
      ...
      <section className="card p-4 sm:p-8">
        ...
        {!hasVehicle || !year ? (
          ...
          <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
            Für diesen Filter sind aktuell keine Funktionen hinterlegt.
          </div>
        ) : (
          <div className="mt-5 space-y-2">
            {platformCodingGroups.map((group) => {
              ...
              const isExpanded =
                activeGroup !== "Alle" || Boolean(normalizedSearch) || expandedGroups.includes(group);
              const panelId = `booking-coding-${group.toLocaleLowerCase("de")}`;
        ...
        {hasVehicle ? (
          <div className="mt-5 rounded-xl bg-slate-50 p-3 sm:mt-5 sm:p-4">
            ...
            <div className="mt-1.5 flex justify-between text-sm text-blue-700">
              ...
              <b>-{formatEuro(discount)} €</b>
            </div>
            {sfdFee > 0 ? (
              <div className="mt-2 flex justify-between border-t pt-2 text-sm text-blue-700">
            ...
            {next ? (
              <p className="mt-2 text-xs leading-5 text-slate-600">
                Noch {formatEuro(Math.max(0, next - subtotal))} € bis zur nächsten Rabattstufe ({
                  next === 50 ? 5 : next === 100 ? 10 : next === 150 ? 15 : 20
                } %).
              </p>
            ) : (
      ...
      {mode === "remote" ? (
        <section className="card p-4 sm:p-8">
          <div className="text-xs font-bold uppercase tracking-[.16em] text-blue-600 sm:text-sm sm:tracking-[.18em]">
            4 · Termin und Zahlung
          </div>
          {selectedVehicle ? (
            <div className="mt-4 rounded-2xl border border-blue-100 p-4 text-sm leading-6 sm:mt-5 sm:p-5 sm:leading-7">
              <b>Remote</b> · {brand} {selectedVehicle.model} · Baujahr {year}<br />
              {selected.length} Codierung(en) · {Math.round(rate * 100)} % Rabatt · <b>{formatEuro(total)} €</b><br />
              70 % vorab: {formatEuro(prepay)} € · 30 % danach: {formatEuro(finalpay)} €
            </div>
          ) : null}
          ...
          <div
            ...
            aria-label="Zahlungsmöglichkeit für Remote-Termine"
          >
            <span>Zahlungsmöglichkeit: </span><b>PayPal</b>
          </div>
          {!hasVehicle ? (
            <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 px-5 py-3 text-sm font-semibold text-slate-500">
              Bitte zuerst ein Fahrzeug auswählen.
            </div>
          ) : isSfd2 ? (
            <div className="mt-6 rounded-xl bg-slate-100 px-5 py-3 font-semibold text-slate-500">
              Terminbuchung für Codierungen deaktiviert
            </div>
          ) : selected.length === 0 ? (
            <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 px-5 py-3 text-sm font-semibold text-slate-500">
              Bitte zuerst mindestens eine Codierung auswählen.
            </div>
          ) : null}
          <div className={`mt-5 grid gap-3 sm:mt-6 ${bookingDisabled ? "" : "sm:grid-cols-2"}`}>
            ...
            {!bookingDisabled ? (
              <a
                href="/zahlung"
                onClick={savePendingBooking}
                className="btn-secondary flex w-full flex-col text-center"
              >
                <span>Termin gebucht? Jetzt 70 % vorauszahlen</span>
                <span className="mt-1 text-sm font-black">Betrag: {formattedPrepay} €</span>
              </a>
            ) : null}
          </div>
          <p className="mt-3 text-xs leading-5 text-slate-500">
            Cal.com öffnet sich in einem neuen Tab. Nach erfolgreicher Terminbuchung hier zurückkehren und mit Schritt 2 zur Zahlungsseite gehen.
          </p>
        </section>
      ) : (
        <>
          ...
          <section className="card p-4 sm:p-8">
            ...
            {selectedVehicle ? (
              <div className="mt-4 rounded-2xl border border-blue-100 p-4 text-sm leading-6 sm:mt-5 sm:p-5 sm:leading-7">
                <b>Vor Ort</b> · {brand} {selectedVehicle.model} · Baujahr {year}<br />
                {selected.length} Codierung(en) · {Math.round(rate * 100)} % Rabatt · <b>{formatEuro(total)} €</b>
              </div>
            ) : null}
            {!hasVehicle ? (
              <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 px-5 py-3 text-sm font-semibold text-slate-500">
            ...
            <div className="mt-5 sm:mt-6">{calButton("w-full text-center sm:w-auto")}</div>
            <p className="mt-3 text-xs leading-5 text-slate-500">
              Name und E-Mail werden direkt von Cal.com bei der Terminbuchung abgefragt.
            </p>
          </section>
        </>
      )}
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-blue-100 bg-white/95 p-3 shadow-[0_-8px_30px_rgba(15,23,42,.08)] backdrop-blur md:hidden">
        <div className="mx-auto grid max-w-md grid-cols-[2fr_1fr] gap-2">
          ...
          <a
            href="#kontakt"
            className="inline-flex min-h-16 items-center justify-center rounded-xl border border-blue-200 bg-white px-3 py-2 text-center text-sm font-bold text-blue-700"
          >
            Direktkontakt
          </a>
        </div>
