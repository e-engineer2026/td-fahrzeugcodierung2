...
export default function BookingConfigurator(){
  ...
  const savePendingBooking=()=>{
    if(typeof window==="undefined" || mode!=="remote" || selected.length===0) return;
  ...
  return <div className="space-y-5 sm:space-y-8">
    ...
    <section className="card p-4 sm:p-8">
      ...
      <div className="mt-4 grid gap-3 sm:gap-4 md:grid-cols-[1fr_2fr]">
        <div>
          <input value={vin} onChange={e=>setVin(e.target.value.toUpperCase().replace(/[^A-HJ-NPR-Z0-9]/g,"").slice(0,17))} maxLength={17} placeholder="FIN optional"/>
    ...
    {mode==="remote" ? <>
      ...
      <section className="card p-4 sm:p-8">
        ...
        <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50 p-4 sm:mt-5 sm:p-5"><b>Remote: PayPal · 70 % vorab / 30 % danach</b><p className="mt-2 text-sm text-slate-600">PayPal.Me: <strong>paypal.me/TiDrechsler</strong></p><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-white p-4"><span className="text-sm text-slate-500">70 % vorab</span><b className="mt-1 block text-2xl">{prepay.toFixed(2)} €</b></div><div className="rounded-xl bg-white p-4"><span className="text-sm text-slate-500">30 % nach Durchführung</span><b className="mt-1 block text-2xl">{finalpay.toFixed(2)} €</b></div></div>{selected.length===0?<div className="mt-4 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-500">PayPal wird nach Auswahl mindestens einer Codierung freigeschaltet.</div>:<a href={paypalUrl} target="_blank" rel="noreferrer" className="btn-primary mt-4 w-full sm:w-auto">Jetzt {prepay.toFixed(2)} € per PayPal vorauszahlen</a>}</div>
        <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm leading-6 text-slate-700 sm:mt-5 sm:p-5 sm:leading-7"><b>Verbindliche Terminbestätigung:</b> Nach der Terminbuchung sind 70 % des Gesamtbetrags über PayPal.Me zu zahlen. <strong>Der Remote-Termin gilt nach Eingang dieser Vorauszahlung als verbindlich bestätigt.</strong> Die verbleibenden 30 % werden nach Durchführung der vereinbarten Codierung fällig.</div>
      </section>
    </> : <>
