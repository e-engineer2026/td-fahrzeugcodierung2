...
import FlashInquiryForm from "../components/FlashInquiryForm";
...
export const metadata: Metadata = {
  ...
  alternates: { canonical: "/steuergeraete-flash" },
};
...
export default function SteuergeraeteFlashPage() {
  return (
    <main className="min-h-screen bg-[#f4f8ff]">
              ...
              <div className="rounded-3xl border border-blue-100 bg-[#f8fbff] p-4 sm:p-5">
                ...
                <ul className="mt-3 space-y-2">
                  {services.map(service => (
                    <li key={service} className="flex gap-3 text-sm leading-5 text-slate-700 sm:text-base">
      ...
      <section className="container-x py-8 sm:py-10">
        <aside className="rounded-3xl border border-amber-200 bg-amber-50 p-5 sm:p-6">
          <div className="flex items-center gap-3 text-amber-900"><CircleAlert className="h-5 w-5" /><h2 className="text-xl font-black">Wichtiger Hinweis</h2></div>
          <p className="mt-3 leading-7 text-amber-950/80">Der Preis wird erst nach technischer Prüfung festgelegt. Entscheidend sind Steuergerät, vorhandener Softwarestand, Datenverfügbarkeit und Arbeitsaufwand.</p>
        </aside>
      </section>
